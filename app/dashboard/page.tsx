...
export default async function DashboardPage() {
	...
	return <main className="dashboard">
		<header className="dashboard-header">
			...
			<Link className="primary" href="/viewer">+ Open PDF</Link>
		</header>
		<div className="dashboard-shell">
			...
			<section className="dashboard-content">
				...
				<section className="dashboard-panel lessons-panel"><div className="panel-heading"><div><p className="eyebrow">YOUR LIBRARY</p><h2>Recent lessons</h2></div><Link className="text-link" href="/documents">View all →</Link></div><div className="lesson-list">{lessons.map(lesson => <Link className="lesson-row" href={lesson.href} key={lesson.title}><span className={`lesson-thumb ${lesson.tone}`}><span>▤</span></span><span className="lesson-copy"><strong>{lesson.title}</strong><small>{lesson.meta}</small></span><span className="lesson-more">•••</span></Link>)}</div></section>
