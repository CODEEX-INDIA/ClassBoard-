...
export function SignInPanel() {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setMessage("Signing in…"); try { const client = getSupabaseBrowserClient(); const result = await client.auth.signInWithPassword(method === "email" ? { email: identifier, password } : { phone: identifier, password }); if (result.error) { setMessage(result.error.message); return; } router.push("/dashboard"); router.refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to sign in."); } };
